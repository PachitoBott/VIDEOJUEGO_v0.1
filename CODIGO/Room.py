...
def _treasure_hitbox_from_sprite_rect(sprite_rect: pygame.Rect) -> pygame.Rect:
    ...
    # Limitar la caja de colisión a un tile para que el jugador pueda
    # acercarse más al cofre sin bloquear tiles adyacentes.
    tile_size = max(4, int(getattr(CFG, "TILE_SIZE", 32)))
    hitbox_w = min(hitbox_w, tile_size)
    hitbox_h = min(hitbox_h, tile_size)
