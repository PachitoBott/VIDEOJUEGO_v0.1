...
class LootNotification:
    ...
    def __init__(
        ...
        *,
        ...
        text_scale = max(0.5, float(text_scale))
        padding_x, padding_y = (10 * text_scale, 6 * text_scale)

        text_surface = font.render(text, True, text_color)
        if text_scale != 1.0:
            w = max(1, int(text_surface.get_width() * text_scale))
            h = max(1, int(text_surface.get_height() * text_scale))
            text_surface = pygame.transform.smoothscale(text_surface, (w, h))
        width = int(text_surface.get_width() + padding_x * 2)
        height = int(text_surface.get_height() + padding_y * 2)
...
class LootNotificationManager:
    ...
    def __init__(
        ...
        *,
        ...
        self.text_scale = max(0.5, float(text_scale))
    ...
    def push(self, message: str) -> None:
        ...
        note = LootNotification(message, self.font, start_pos=start_pos, text_scale=self.text_scale)
